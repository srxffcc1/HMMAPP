...
public class MessageDisAdapter extends BaseQuickAdapter<MonMessageOtherHelper, BaseViewHolder> {


    private String fragmentType;

    public MessageDisAdapter() {
        this(R.layout.home_item_activity_messagedis);
    }

    private MessageDisAdapter(int layoutResId) {
        super(layoutResId);
    }
    ...
    protected void convert(BaseViewHolder helper, final MonMessageOtherHelper item) {
        ...
        if("收到的".equals(fragmentType)){
            like.setVisibility(View.INVISIBLE);
            toReview.setVisibility(View.VISIBLE);
        }else {
            like.setVisibility(View.GONE);
            toReview.setVisibility(View.GONE);
        }
        ...
        if(item.postingInfoByUrlStatusAndPostingIdDTO!=null&&item.hmmCityRecordByMemberIdDTOS.replyType==2){
            if(!TextUtils.isEmpty(item.postingInfoByUrlStatusAndPostingIdDTO.url)){
                ...
                com.healthy.library.businessutil.GlideCopy.with(postImg.getContext())
                        ...
                        .into(postImg);
//            if(item.postingInfoByUrlStatusAndPostingIdDTO.url){
...
//            }
        ...
        if(item.memberStateDTO!=null){
            if(!TextUtils.isEmpty(item.memberStateDTO.memberState)){
    ...
    public void setType(String fragmentType) {

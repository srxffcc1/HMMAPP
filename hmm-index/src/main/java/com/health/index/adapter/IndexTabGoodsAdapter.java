...
import com.healthy.library.interfaces.Corner;
...
public class IndexTabGoodsAdapter extends BaseAdapter<ActGoodsItem> {
    ...
    public IndexTabGoodsAdapter() {
        this(R.layout.index_mon_goods);
    }

    private IndexTabGoodsAdapter(int viewId) {
        super(viewId);
    }
...
//                0);
    ...
    public void onBindViewHolder(@NonNull BaseHolder baseHolder, int position) {
        ...
        final int radius = TransformUtil.newDp2px(context, 8);
        ...
        if (!TextUtils.isEmpty(imageScalemap.get(actGoodsItem.filePath))) {
            ...
            Glide.with(context).load(actGoodsItem.filePath)

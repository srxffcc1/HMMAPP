...
public class SettingsCompat {
    ...
    private static boolean manageDrawOverlaysForRom(Context context) {
        if (RomUtil.isMiui()|| 1==RomIdentifier.getRom().compareTo(Rom.MIUI)) {
            return manageDrawOverlaysForMiui(context);
        }
        if (RomUtil.isEmui()|| 1==RomIdentifier.getRom().compareTo(Rom.EMUI)) {
            return manageDrawOverlaysForEmui(context);
        }
        if (RomUtil.isFlyme()|| 1==RomIdentifier.getRom().compareTo(Rom.Flyme)) {
            return manageDrawOverlaysForFlyme(context);
        }
        if (RomUtil.isOppo()|| 1==RomIdentifier.getRom().compareTo(Rom.ColorOS)) {
            return manageDrawOverlaysForOppo(context);
        }
        if (RomUtil.isVivo()|| 1==RomIdentifier.getRom().compareTo(Rom.FuntouchOS)) {

...
public class ActZBlock2Adapter extends BaseMultiItemAdapter<MainBlockModel> {
    ...
    public ActZBlock2Adapter() {
        ...
        addItemType(6, R.layout.dis_item_block_p6);//联盟单行通栏列表
        addItemType(7, R.layout.dis_item_block_p7);//通栏介绍+联盟组合列表
    }
    ...
    public void onBindViewHolder(@NonNull BaseHolder holder, @SuppressLint("RecyclerView") int position) {
        ...
        int nextindex = -1;
        try {
            if (getItemType(position) != 6 || getItemType(position) != 7) {
                banner = holder.itemView.findViewById(R.id.act_bottom_banner);
...
//            e.printStackTrace();
    ...
    private void buildPostList7Item(LinearLayout postList, final MainBlockModel.AllianceMerchant item) {
        postList.removeAllViews();
        if (item.detailList != null && item.detailList.size() > 0) {
            View view = LayoutInflater.from(context).inflate(R.layout.dis_item_block_child7, postList, false);
...
//                    e.printStackTrace();
            ...
            banner.setOnBannerListener(new OnBannerListener() {
                ...
                public void OnBannerClick(Object data, int position) {
                    ...
                    new AdPresenterCopy(context).posAd(new SimpleHashMapBuilder<String, Object>().puts("id", adModel.id + ""));
...
//            banner.setIndicator(new CircleIndicator(mContext));
    ...
    private void buildPostList(LinearLayout postList, final MainBlockModel item) {
        ...
        for (int i = 0; i < item.childList.size(); i++) {
            ...
            GlideCopy.with(context).load(itemchild.iconUrl)
    ...
    private void buildGoodsHlist(LinearLayout goodsHList, final MainBlockModel item) {
        ...
        for (int i = 0; i < item.childList.size(); i++) {
            ...
            if (finalItemchild.themeTag.contains("http")) {
                ...
                GlideCopy.with(context).load(itemchild.themeTag)
                        .placeholder(R.drawable.img_1_1_default2)
                        .error(R.drawable.img_1_1_default)
    ...
    private void buildchild5items(LinearLayout goodsList, final MainBlockModel item) {
        ...
        for (int i = 0; i < item.detailList.size(); i++) {
            ...
            if (mainBlockDetailModel == null) {
                ...
                GlideCopy.with(context).load(mainBlockDetailModel.getGoodsIcon())
                        .placeholder(R.drawable.img_1_1_default2)
                        .error(R.drawable.img_1_1_default)
    ...
    private void buildType2(View view, final MainBlockModel item, int position) {
...
//                e.printStackTrace();
            ...
            if (item.detailList == null || item.detailList.size() == 0 || mainBlockDetailModel == null || item.detailList.get(i).goodsDTO == null) {
                ...
                GlideCopy.with(context).load(item.detailList.get(i).goodsDTO.getGoodsIcon())
                        .placeholder(R.drawable.img_1_1_default2)
                        .error(R.drawable.img_1_1_default)
    ...
    private void buildLeftGrid(LinearLayout leftGrid, final MainBlockModel item) {
        ...
        GlideCopy.with(context).load(item.detailList.get(0).goodsDTO.getGoodsIcon())
                .placeholder(R.drawable.img_1_1_default2)
                .error(R.drawable.img_1_1_default)
    ...
    private void buildType1(View view, final MainBlockModel item, int position) {
        ConstraintLayout topLL;
        ImageTextView topTitle;
        ImageTextView topSecondTitle;
        ImageTextView topMore;
...
//                });
                ...
                GlideCopy.with(context).load(mainBlockDetailModel.goodsDTO.getGoodsIcon())
                        .placeholder(R.drawable.img_1_1_default2)
                        .error(R.drawable.img_1_1_default)

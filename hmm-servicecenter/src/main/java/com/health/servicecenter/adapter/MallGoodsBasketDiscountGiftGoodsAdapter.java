 ...
 * 门店组
 */
public class MallGoodsBasketDiscountGiftGoodsAdapter extends BaseAdapter<ActVip> {
    ...
    public MallGoodsBasketDiscountGiftGoodsAdapter() {
        this(R.layout.service_item_goodsbasket_discount_giftall);
    }

    private MallGoodsBasketDiscountGiftGoodsAdapter(int viewId) {
        super(viewId);
    }
    ...
    public void onBindViewHolder(@NonNull BaseHolder baseHolder, final int i) {
        ...
        discountGoodsOrgAll = (GridLayout) baseHolder.itemView.findViewById(R.id.discountGoodsALL);
        addFunctions(context,discountGoodsOrgAll,getModel().getGoodsListWithGift());
    }
    private void addFunctions(final Context context, final GridLayout gridLayout, final List<ActVip.SaleInfo> list) {
        ...
        if(list!=null&&list.size()>0){
            ...
            for (int i = 0; i < list.size(); i++) {
                View view = LayoutInflater.from(context).inflate(R.layout.service_item_goodsbasket_discount_orgbasketwithselect_itemgoodsgift, gridLayout, false);
...
//                    goodsMoney.setText("¥"+ FormatUtils.moneyKeep2Decimals(saleInfo.FactPrice));
                }

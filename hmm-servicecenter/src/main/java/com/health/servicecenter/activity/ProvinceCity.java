...
import com.health.servicecenter.adapter.ChoseWhereAdapter;
import com.health.servicecenter.contract.ProvinceCityContract;
import com.health.servicecenter.decoration.ProvinceDecoration;
import com.health.servicecenter.presenter.ProvinceCityPresenter;
...
@Route(path = ServiceRoutes.SERVICE_PROVINCECITY)
public class ProvinceCity extends BaseActivity implements ProvinceCityContract.View,
        ...
        mTvLocation.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (!"定位中".equals(mTvLocation.getText().toString())) {
                    ...
                    intent.putExtra("province", LocUtil.getProvinceName(mContext,SpKey.LOC_ORG));
                    intent.putExtra("city", LocUtil.getCityName(mContext,SpKey.LOC_ORG));
...
//            mLocationClient.startLocation();
    ...
    public void onChoseClick(ProvinceCityModel provinceCityModel) {
        ...
        if (nowType == 2) {
            ...
            String city = "";
            //因为只需要城市名  所以排除掉四个直辖市
            if ("北京市".equals(mProvModel.getName())) {
                city = mProvModel.getName();
            } else if ("天津市".equals(mProvModel.getName())) {
                city = mProvModel.getName();
            } else if ("重庆市".equals(mProvModel.getName())) {
                city = mProvModel.getName();
            } else if ("上海市".equals(mProvModel.getName())) {
                city = mProvModel.getName();
            } else {
                city = mCityModel.getName();
            }
//            intent.putExtra("provinceCode",mProvModel.getAreaNo());
//            intent.putExtra("cityCode",mCityModel.getAreaNo());
//            intent.putExtra("areasCode",mStreetModel.getAreaNo());
//            intent.putExtra("longitude",mProvModel.getAreaNo());
//            intent.putExtra("latitude",mProvModel.getAreaNo());
            Intent intent = new Intent();
            intent.putExtra("province", mProvModel.getName());

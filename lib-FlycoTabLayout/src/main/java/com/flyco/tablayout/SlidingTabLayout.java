...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mIndicatorRes=ta.getResourceId(R.styleable.SlidingTabLayout_tl_indicator_res,-1);
        if(mIndicatorRes!=-1){//说明有res下标则 需要对几个值进行重新设置
            mIndicatorDrawable=getResources().getDrawable(mIndicatorRes);
            mIndicatorHeight=(int)((mIndicatorDrawable.getIntrinsicHeight()*1.0/mIndicatorDrawable.getIntrinsicWidth())*mIndicatorWidth);
        }
        ta.recycle();
    }
...
//        }
    ...
    public void setViewPager(ViewPager vp, final List<String> titles) {
        ...
        if(mViewPager!=null){
            if (titles.size() != mViewPager.getAdapter().getCount()) {
                throw new IllegalStateException("Titles length must be the same as the page count !");
            }
            this.mViewPager.removeOnPageChangeListener(this);
            this.mViewPager.addOnPageChangeListener(this);
    ...
    public void fixViewPager() {//如果缺省ViewPager 有问题 特别是放在adapter里
...
//            ViewGroup targetContainer = (ViewGroup) targetParent;//当前页面得父容器
//            int index = targetContainer.indexOfChild(targetView);//当前页面在父容器中得位置
//            ViewGroup.LayoutParams targetParams = targetView.getLayoutParams();//当前页面在父容器中的页面参数
//            targetContainer.removeView(targetView);//将当前页面移除 以下将增加一个代理容器
//            LinearLayout linearLayout = new LinearLayout(targetContainer.getContext());
//            linearLayout.setOrientation(LinearLayout.VERTICAL);
//            linearLayout.setGravity(Gravity.CENTER);
//            targetContainer.addView(linearLayout, index, targetParams);
//            targetView.setLayoutParams(new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT,0,1));
//            linearLayout.addView(targetView);
//            final ViewPager viewPager=new ViewPager(targetContainer.getContext());
...
//                }
//            },300);
//            viewPager.setLayoutParams(new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT,2));
//            linearLayout.addView(viewPager);//此处需要增加一个viewpager
...
//        }
    ...
    public void notifyDataSetChanged() {
        ...
        if(mViewPager!=null){
            ...
            this.mTabCount = mTitles == null ? mTitles.size() : mTitles.size();
        }
        ...
        for (int i = 0; i < mTabCount; i++) {
            ...
            if(mViewPager!=null){
                ...
                pageTitle = mTitles == null ?  mTitles.get(i) : mTitles.get(i);
            }
            if(pageTitle==null){
                pageTitle="";
            }
    ...
    public void addNewTab(String title) {
        ...
        if(mViewPager!=null){
             ...
             pageTitle = mTitles == null ?  mTitles.get(mTabCount) : mTitles.get(mTabCount);
        }
    ...
    private void addTab(final int position, String title, View tabView) {
        ...
        /** 每一个Tab的布局参数 */
        //System.out.println("mTabSpaceEqual:"+mTabSpaceEqual);
...
//            v.setPadding((int) mTabPadding, v.getPaddingTop(), (int) mTabPadding, v.getPaddingBottom());
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
         ...
         */
//        System.out.println("滚动选中:"+position+":"+positionOffset);

...
/**
 * @author: luoyang
 * @verson: [AppZxs，2018/8/23 15:29]
 * @description: 沉浸式状态栏工具栏
 */
public class StatusBarUtil {
     ...
     *
     * @param activity
     * @param isVisiable View.Visable
     */
    public static void setStatusVisalbe(Activity activity, int isVisiable) {
        if (activity == null) {
            return;
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            activity.getWindow().getDecorView().setSystemUiVisibility(isVisiable);
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            activity.getWindow()
                    .setFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS, WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
        }

...
import com.healthy.library.utils.CharUtil;
...
public class CollapsedTextView extends AppCompatTextView implements View.OnClickListener {
    ...
    private ExpandedClick expandedClick = null;
    ...
    private void formatCollapsedText(TextView.BufferType type, CharSequence text) {
        ...
        if (layout != null) {
            mCurrentLine = layout.getLineCount();
            if (mCurrentLine <= mCollapsedLines) {
                super.setText(mOriginalText, type);
            } else {
                // 最后一行的开始字符位置
    ...
    private void setSpan(SpannableStringBuilder spannable) {
        Drawable drawable = mCollapsedDrawable;
    ...
    public interface ExpandedClick {
        void click(boolean mIsExpanded);
    }

    public void setExpandedClick(ExpandedClick click) {
        this.expandedClick = click;
    }
    ...
    private class ExpandedClickableSpan extends ClickableSpan {
        ...
        public void onClick(View widget) {
            ...
            if (expandedClick != null) {
                expandedClick.click(mIsExpanded);
            }
        }

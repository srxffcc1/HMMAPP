...
public class TongLianPhonePresenterCopy implements TongLianPhoneContract.Presenter {

    private Context mContext;
    private TongLianPhoneContract.View mView;

    public TongLianPhonePresenterCopy(Context context, TongLianPhoneContract.View view) {
        mContext = context;
        mView = view;
    }
    ...
    public void getTongLianPhoneStatus(Map<String, Object> map) {
        if(TextUtils.isEmpty(SpUtils.getValue(LibApplication.getAppContext(), SpKey.TOKEN))){
            return;
        }
    ...
    public void bindPhone(Map<String, Object> map) {
        map.put(Functions.FUNCTION, "allin_10003");
        ObservableHelper.createObservable(mContext, map)
                .subscribe(new StringObserver(mView, mContext,
                        false) {
                    @Override

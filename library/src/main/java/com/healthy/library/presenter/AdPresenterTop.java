...
public class AdPresenterTop implements AdContract.Presenter {

    private Context mContext;
    private AdContract.View mView;

    public AdPresenterTop(Context context, AdContract.View view) {
        mContext = context;
        mView = view;
    }
        ...
        ObservableHelper.createObservable(mContext, map)
                .subscribe(new NoStringObserver(mView, mContext,
                    ...
                    protected void onGetResultSuccess(String obj) {
                        ...
                        List<AdModel> adModelList=resolveAdListData(obj);
                        if(adModelList==null||adModelList.size()==0){
                            try {
                                mView.onSucessGetAds(new SimpleArrayListBuilder<AdModel>().adds(new AdModel("-1",Integer.parseInt(map.get("type").toString()),Integer.parseInt(map.get("triggerPage").toString()))));
                            } catch (NumberFormatException e) {
                                e.printStackTrace();
                            }
                        }else {
                            mView.onSucessGetAds(adModelList);
        ...
        ObservableHelper.createObservable(mContext, map)
                .subscribe(new NoStringObserver(mView, mContext,
                    ...
                    protected void onFailure() {
                        super.onFailure();
                        try {
                            mView.onSucessGetAds(new SimpleArrayListBuilder<AdModel>().adds(new AdModel("-1",Integer.parseInt(type),Integer.parseInt(triggerPage))));
                        } catch (NumberFormatException e) {
                            e.printStackTrace();
                        }
                    }

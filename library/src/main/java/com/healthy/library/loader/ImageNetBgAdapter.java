 ...
 * 自定义布局，网络图片
 */
public class ImageNetBgAdapter extends BannerAdapter<String,ImageBgHolder> {

    private List<ColorInfo> colorList=new ArrayList<>();
    private float mRadius;
    private Palette palette;
    public ImageNetBgAdapter(List<String> mDatas, float radius, List<ColorInfo> colorList) {
        super(mDatas);
        mRadius = radius;
    ...
    @Override
    public ImageBgHolder onCreateHolder(ViewGroup parent, int viewType) {
...
//                ViewGroup.LayoutParams.MATCH_PARENT,
//                ViewGroup.LayoutParams.MATCH_PARENT));
        return new ImageBgHolder(view,mRadius);
    }

    @Override
    public void onBindView(ImageBgHolder holder, final String data, int position, int size) {

            com.healthy.library.businessutil.GlideCopy.with(holder.itemView).asBitmap().load(data).diskCacheStrategy( DiskCacheStrategy.NONE ).listener(new RequestListener<Bitmap>() {
...
//                .into(holder.imageView);
    ...
    public int getPerfectColor(int position) {
        return colorList.get(position).getPerfectColor();
    }

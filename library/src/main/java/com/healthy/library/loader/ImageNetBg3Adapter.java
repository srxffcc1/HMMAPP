 ...
 * 自定义布局，网络图片
 */
public class ImageNetBg3Adapter extends BannerAdapter<String,ImageBgHolder> {

    private List<ColorInfo> colorList=new ArrayList<>();
    private float mRadius;
    private Palette palette;
    public ImageNetBg3Adapter(List<String> mDatas, float radius, List<ColorInfo> colorList) {
        super(mDatas);
        mRadius = radius;
...
//                ViewGroup.LayoutParams.MATCH_PARENT));
    ...
    public void onBindView(ImageBgHolder holder, final String data, int position, int size) {
            ...
            com.healthy.library.businessutil.GlideCopy.with(holder.itemView).asBitmap().load(data).listener(new RequestListener<Bitmap>() {

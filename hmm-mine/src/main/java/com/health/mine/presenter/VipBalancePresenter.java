...
public class VipBalancePresenter implements VipBalanceContract.Presenter {
    private Context mContext;
    private VipBalanceContract.View mView;

    public VipBalancePresenter(Context context, VipBalanceContract.View view) {
        mContext = context;
        mView = view;
    }
        ...
        ObservableHelper.createObservable(mContext, map)
                .subscribe(new StringObserver(mView, mContext, false, false) {
                    ...
                    protected void onGetResultSuccess(String obj) {
                        ...
                        mView.onGetBalanceListSuccess(resolveData(obj));
                    }
    ...
    @Override
    public void getBalanceLogList(Map<String, Object> map) {
        map.put(Functions.FUNCTION, "20004");
        ObservableHelper.createObservable(mContext, map)
                .subscribe(new StringObserver(mView, mContext, false, false) {
                    @Override
                    protected void onGetResultSuccess(String obj) {
                        super.onGetResultSuccess(obj);
                        mView.onGetBalanceLogListSuccess(resolveRefreshData(obj), resolveListData(obj));
                    }
    ...
    private List<BalanceModel> resolveData(String obj) {
    ...
    private List<BalanceListModel> resolveListData(String obj) {
        List<BalanceListModel> result = new ArrayList<>();
        try {
            ...
            Gson gson = builder.create();
            Type type = new TypeToken<List<BalanceListModel>>() {
            }.getType();
            result = gson.fromJson(userShopInfoDTOS, type);
        } catch (Exception e) {
            e.printStackTrace();
